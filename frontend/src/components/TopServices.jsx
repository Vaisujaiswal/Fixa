...
//             <div onClick={() => {navigate(`/booking/${item._id}`); scrollTo(0, 0)}} key={index} className="rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 hover:shadow-2xl dark:hover:shadow-2xl">
...
const TopServices = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-4 my-24 md:mx-10">
      {/* Heading */}
      <h1 className="text-3xl font-bold text-[#222325] dark:text-white">
        Top Services to Book
      </h1>
      <p className="sm:w-1/2 text-center text-sm text-gray-700 dark:text-gray-400">
        Find the best service providers for your home. Happiness starts here!
      </p>

      {/* Service Cards */}
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mt-8">
        {serviceProviders.slice(0, 8).map((item, index) => (
          <div
            key={index}
